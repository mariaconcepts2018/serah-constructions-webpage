...
>
    <div className={` mx-auto w-full xl:w-1/4 lg:w-1/2 lg:mr-0 h-screen md:h-1/2 max-h-screen shadow-sm z-300 flex flex-col justify-between shadow-black shadow-xl bg-neutral-700`}>
        <div className="flex justify-between items-start bg-neutral-800 w-full p-4">
          <div className="block">
          <h1 className="text-lg">Serah Constructions</h1>
            ...
            <button className="hover:bg-neutral-600 rounded-full text-4xl text-center transition"
                onClick={handleModal}
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="m-2" width="0.75em" height="0.75em" viewBox="0 0 24 24"><path fill="currentColor" d="m18 12l.354-.354l.353.354l-.353.354zm-12 .5a.5.5 0 0 1 0-1zm8.354-4.854l4 4l-.708.708l-4-4zm4 4.708l-4 4l-.708-.708l4-4zM18 12.5H6v-1h12z"/></svg>
            </button>
        </div>
        
                <div ref={messagesRef} className="flex flex-col-reverse w-full h-full overflow-y-auto   gap-y-6 p-4">

          {messages.length === 0?
        <div className=" text-md text-left p-4 self-start">
          <p className="shadow-lg xl:py-3 py-2 px-4 text-left w-full block bg-neutral-400 text-neutral-800 rounded-e-full rounded-es-full">How can we assist you?</p>
        </div> 
        :
<>
          {messages.map((msg, i) => (
...
<div className={`shadow-lg xl:py-3 py-2 px-4 text-left ${msg.isAdmin? 'rounded-e-full rounded-es-full bg-neutral-400 text-neutral-800' :'rounded-s-full rounded-se-full bg-primary-200 text-neutral-800'}`}>
{msg.text}
</div>
</div>
      ...
      <form onSubmit={sendMessage} className="block w-full flex justify-between p-4 bg-neutral-700">
          <input
            value={text}
