...
export default function ChatBox({ openModal, handleModal }) {
  ...
  return (
    ...
    >
      <div
        className={` mx-auto w-full xl:w-1/3 2xl:w-1/4 lg:w-1/2 lg:mr-0 h-screen md:h-1/2 max-h-screen shadow-sm z-300 flex flex-col justify-between shadow-black shadow-xl bg-neutral-700`}
      >
        <div className="flex justify-between items-start bg-neutral-800 w-full p-4">
          ...
          <button
            className="hover:bg-neutral-600 rounded-full text-4xl text-center transition"
            onClick={handleModal}
          >
            <svg
        ...
        <div
          ref={messagesRef}
          className="flex flex-col-reverse w-full h-full overflow-y-auto   gap-y-6 p-4"
        >
          {messages.length === 0 ? (
        ...
        <form
          ...
          className="block w-full flex justify-between p-4 bg-neutral-700"
        >
          <input
            value={text}
